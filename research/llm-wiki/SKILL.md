...
① **Capture the raw source:**
   - URL → prefer `web_extract` to get markdown, save to `raw/articles/`
   - PDF → use `web_extract` (handles PDFs), save to `raw/papers/`
   - Pasted text → save to appropriate `raw/` subdirectory
   - Name the file descriptively: `raw/articles/karpathy-llm-wiki-2026.md`
   - If direct arXiv API ingestion (`export.arxiv.org/api/query`) times out or returns HTTP 429, fall back to `web_extract` on `https://arxiv.org/abs/<id>` and save the extracted abstract/metadata page as the raw source instead of blocking the ingest.
