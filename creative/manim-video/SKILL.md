...
Requires: Python 3.10+, LaTeX (`texlive-full` on Linux, `mactex` on macOS), and ffmpeg. Reference docs tested against Manim CE v0.20.1.

### Manim Installation (venv bootstrap)

Manim is **not installed system-wide** on most environments. Bootstrap it in a dedicated venv:
...
On this environment, always invoke via:
```bash
~/.venvs/manim/bin/manim [args]
```

**Never** `pip install manim` without a venv on systems with PEP 668 (Debian/Ubuntu 22.04+, Fedora 37+) — it will refuse and refuse without `--break-system-packages`. Use the venv approach every time.

Verify: `~/.venvs/manim/bin/manim --version`
...
**Single-scene narrative film** (one continuous story, one topic):
```bash
manim -ql script.py MyNarrativeFilm  # draft — all content in ONE .mp4
```
No stitching needed. For films ≥5 min, render in background with `notify_on_complete=true`.

**Multi-scene modular film** (independent scenes):
```bash
manim -ql script.py Scene1 Scene2 Scene3  # draft
manim -qh script.py Scene1 Scene2 Scene3  # production
```
