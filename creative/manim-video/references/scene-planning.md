...
## Single vs. Multiple Scenes

### Single-Scene Narrative Film (preferred for story-driven content)
For a **continuous narrative** (one topic, one story, one arc), use ONE `Scene` class with internal `_teilN_name()` helper methods. This avoids scene transitions that feel like slides, re-initializing the camera, and manual ffmpeg stitching:

```python
class MyNarrativeFilm(Scene):
    def construct(self):
        self._teil1_introduction()
        self._teil2_core_concept()
        self._teil3_solution()
        self._teil4_visualization()
        self._teil5_summary()
```

- All content is in one `.mp4` — no ffmpeg stitching needed
- Render: `manim -ql script.py MyNarrativeFilm`
- For 5+ min films: always render in background with `notify_on_complete=true`
- ~174 animations ≈ 4.5 min at 480p. Scale `run_time` to taste.

### Multiple Scene Classes (better for modular/independent content)
Use separate scene classes when each scene is independently useful, reorderable, or needs different quality levels. Then stitch with ffmpeg.
