...
- If an internal HTTPS MCP endpoint works with `curl -k` but `mcporter` reports `unable to verify the first certificate`, the server is reachable but the certificate chain is not trusted locally. Prefer fixing the CA trust/server cert; only use insecure TLS bypasses for debugging.
- Reliable local fix: create a CA bundle that includes the missing intermediate and run mcporter with `NODE_EXTRA_CA_CERTS=/path/to/custom-ca-bundle.pem`. This preserves TLS verification while allowing Node to trust the internal chain.
