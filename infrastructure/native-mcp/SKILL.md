...
description: >-
  Complete guide to MCP in Hermes: the built-in native MCP client (config,
  transport types, security, troubleshooting), the mcporter CLI for ad-hoc
  server calls, and the AIP docs MCP server for library documentation search.
author: Hermes Agent
date: 2026-04-30
...
see the [mcporter](#part-2-mcporter-cli) section below.

### Prerequisites
- **mcp Python package** -- optional dependency; install with `pip install mcp`
...
### How It Works
1. On startup, reads `mcp_servers` from config
2. For each server, spawns a connection in a dedicated background event loop
3. Initializes the MCP session and calls `list_tools()` to discover tools
4. Registers each tool with prefix `mcp_{server_name}_{tool_name}`
5. Auto-injects into all platform toolsets

### Security
- **Environment variable filtering:** Only safe baseline variables are inherited
  by stdio servers. Sensitive vars must be explicitly added via `env:` config.
...
- Python code from these docs → load `python-mcp-docs-first`
- Dask-specific → load `dask-mcp-docs-first`
- pandas + Datashader plotting → load `pandas-datashader-mcp-docs-first`
...
```bash
# Step 1: Check TLS connectivity
curl -v https://<server-host>/mcp --max-time 10 2>&1 | grep -E "(SSL|certificate|verify|TLS)"

# Step 2: MCP initialize handshake (required: both JSON and SSE Accept headers)
...
# Step 3: List tools (if init succeeded)
curl -s -X POST https://<server-host>/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/list","params":{}}'

# Step 4: Test an actual tool
curl -s -X POST https://<server-host>/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
- Increase `connect_timeout` (default 60s) in server config
- Verify the `mcp` Python package includes HTTP support: `pip show mcp` should list `streamable_http`
- Check startup logs: `grep -i mcp ~/.hermes/logs/gateway.log`
